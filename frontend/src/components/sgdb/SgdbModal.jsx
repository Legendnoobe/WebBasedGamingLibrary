    ...
    return (
        <div className="modal-overlay" onClick={onClose} style={{ zIndex: 200 }}>
            <div className="glass-panel"
                style={{ width: '820px', height: '82vh', borderRadius: '20px', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 30px 80px rgba(0,0,0,0.7)' }}
                onClick={e => e.stopPropagation()}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 24px', borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'rgba(0,0,0,0.25)' }}>
                    <h2 style={{ margin: 0, fontSize: '18px' }}>🎨 {t('sgdb.title')} — {game?.name}</h2>
                    <button className="modal-close" style={{ position: 'static' }} onClick={onClose}><X size={20} /></button>
                </div>

                <div style={{ display: 'flex', gap: '8px', padding: '16px 24px', borderBottom: '1px solid rgba(255,255,255,0.05)', background: 'rgba(0,0,0,0.15)' }}>
                    {phase === 'images' && (
                    ...
                    <input id="sgdb-search-input" value={searchText} onChange={e => setSearchText(e.target.value)}
                        ...
                        style={{ flex: 1, padding: '10px 14px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: '#fff', fontSize: '14px' }} />
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '16px 24px' }}>
                    ...
                    {!loading && phase === 'search' && results.map(r => (
                        <div key={r.id} tabIndex="0" onClick={() => handleSelectGame(r.id)} onKeyDown={e => e.key === 'Enter' && handleSelectGame(r.id)}
                            style={{ padding: '12px 16px', background: 'rgba(255,255,255,0.04)', marginBottom: '6px', borderRadius: '10px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '12px', transition: 'background 0.15s' }}
                            onMouseEnter={e => e.currentTarget.style.background = 'rgba(107,76,255,0.12)'}
                    ...
                    {!loading && phase === 'images' && images && (
                        <div>
                            <h3 style={{ borderBottom: '1px solid rgba(255,255,255,0.08)', paddingBottom: '10px', marginBottom: '14px', fontSize: '15px' }}>
                                {t('sgdb.grids')}
                            </h3>
...
function ImageThumb({ src, label, width, height, onClick }) {
    return (
        <div tabIndex="0" onClick={onClick} onKeyDown={e => e.key === 'Enter' && onClick()}
            ...
            <img src={src} style={{ width, height, objectFit: 'cover', display: 'block', background: 'rgba(0,0,0,0.4)' }} loading="lazy" alt="" />
